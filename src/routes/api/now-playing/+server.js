...
export async function GET() {
    try {
        ...
        if (!tracks || tracks.length === 0) {
            return json({ isPlaying: false }, { headers: cacheHeaders });
        }
        ...
        const image =
            images.find(/** @param {any} img */ (img) => img.size === 'large')?.['#text'] ||
            images.find(/** @param {any} img */ (img) => img.size === 'medium')?.['#text'] ||
            images[images.length - 1]?.['#text'] ||
            '/favicon-small.png'; // fallback
        ...
        return json(
            {
                ...
                url:
                    track.url ||
                    `https://www.last.fm/music/${encodeURIComponent(track.artist?.['#text'] || track.artist)}/_/${encodeURIComponent(track.name)}`,
                timestamp: track.date?.uts ? new Date(parseInt(track.date.uts) * 1000) : null,
                isRecent:
                    !isCurrentlyPlaying && track.date?.uts
